...
import { contentStoreIntroApi } from '@/api/content'
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await contentStoreIntroApi()
...
<template>
  <view class="shop-intro">
    <NavHead :title="pageData?.heroTitle || '门店介绍'" :show-back="true" />
      ...
      <view style="padding: 24rpx;">
        ...
        <view class="hero-card">
          <view class="hero-cover">
            ...
            <view class="hero-title">{{ pageData?.heroTitle || '门店介绍' }}</view>
            <view class="hero-subtitle">{{ pageData?.heroSubtitle || '探索门店特色' }}</view>
          </view>
        </view>
        ...
        <view class="card images-card" v-if="(pageData?.galleryImages?.length || 0) > 0 || pageData === null">
          <view class="card-title">{{ pageData?.galleryTitle || '门店风采' }}</view>
